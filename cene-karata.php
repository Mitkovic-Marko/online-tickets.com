...
<html lang="en">
<head>
    <title> | Izmena cene karata</title>
...
<nav class="navbar navbar-expand-lg navbar-light" style="background: rgba(174, 220, 239, 0.6); backdrop-filter: blur(5px);">
  ...
  <div class="container-fluid py-5 text-center"> 
    ...
    <h1 class="  display-10 fw-bold" style="color:black; text-align:center;">IZMENA CENE KARATA </h1>  <br> <br> 
    <form class="row g-3 " action="cene-karata.php" method="POST">
        
    <div class="col-md-4">
        <b style="font-size:20px;">Tip karte</b>
        <select style="text-align:center;" class="form-select" name="trenutna" onchange="updatePrice(this)">
    ...
    ?>
        ...
        <div class="col-md-4">
                <label for="cena" style="font-size:20px;"><b>Cena</b></label> 
                <input style="text-align:center;" type="text" class="form-control" id="cena" name="cena" readonly>
        </div>
        <div class="col-md-4">
                <label for="novakol" style="font-size:20px;"><b>Nova cena:</b></label>
                <input type="number" style="text-align:center;" class="form-control" id="novacena" name="novacena" min="0">
        </div> <br>
        <div class="col-md-12">
        <button type="submit" class="btn btn-dark" name="izmenakol">Promeni cenu</button>
        </div>
</form>
    </div>
    <?php
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['izmenakol'])) {
    ...
    if (isset($_POST['trenutna'])) {
        ...
        $selectedCena = $_POST['trenutna'];

        // Provera da li je polje "Nova količina" prazno
        if (empty($novaCena)) {
            echo '<div class="alert alert-danger text-center" role="alert">
                    Polje "Nova cena" mora biti popunjeno.
                  </div>';
        } else {
            // SQL upit za ažuriranje količine u bazi podataka
            $sql = "UPDATE karta SET cena = '$novaCena' WHERE id_karte = '$selectedCena'";

            if (mysqli_query($conn, $sql)) {
                echo '<div class="alert alert-success text-center" role="alert" >
                        Cena je uspešno ažurirana u bazi podataka.
                      </div>';
            } else {
        ...
        echo '<div class="alert alert-danger  text-center " role="alert">
                Morate izabrati kartu i popuniti polje "Nova cena!".
              </div>';
    }
